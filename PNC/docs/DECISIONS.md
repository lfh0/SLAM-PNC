...
运行态实测`solver_result=-1009`（线搜索达到最大评估次数）在381次迭代、约5.19秒后返回。当前MINCO核心将该返回码暂时当作可继续后验验证的中间解；本次随后被`REFERENCE_DEVIATION_LIMIT`拒绝，证明其不是可直接发布的局部轨迹。后续必须将`-1009`归类为优化失败，并增加局部MINCO独立300~500 ms墙钟预算；时间预算触发时协作取消并由LA-05执行既定降级，而不是继续占用局部重规划链路数秒。
