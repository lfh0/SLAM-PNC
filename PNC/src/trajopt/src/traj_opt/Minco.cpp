...
namespace plan_manage
{
      ...
      std::vector<int> singuls,double now, double help_eps)
  {
    ...
    Eigen::VectorXd x;//x是由两部分组成一: 2*(piece_num - 1)的搜索出来的pos; 二:timePerPiece * piece_nums
    x.resize(variable_num_);
    int offset = 0;
    //memcpy()的意义是把initInnerPts[i]开始的一段内存拷贝到x.data()+offset
    //initInnerPts[i].data()表示initInnerPts[i]的底层数据指针
    for(int i = 0; i<trajnum; i++){
  ...
  double PolyTrajOptimizer::costFunctionCallback(void *func_data, const Eigen::VectorXd &x, Eigen::VectorXd &grad)
  {
    ...
    */
    ...
    // cout << "arraygradT_container 111: " << arraygradT_container[0] << endl;
    // Eigen::VectorXd gradt;
    for(int trajid = 0; trajid < opt->trajnum; trajid++){
      double smoo_cost = 0;
      Eigen::VectorXd obs_surround_feas_qvar_costs(3);
      obs_surround_feas_qvar_costs.setZero();
      opt->jerkOpt_container[trajid].generate(P_container[trajid], arrayT_container[trajid]);//P_container存的是所有的路径点，arrayT_container存的是轨迹的时间
      opt->initAndGetSmoothnessGradCost2PT(arraygradT_container[trajid], smoo_cost, trajid); // Smoothness cost   
      // cout << "arraygradT_container 222: " << arraygradT_container[0] << endl;
      opt->addPVAGradCost2CT(arraygradT_container, obs_surround_feas_qvar_costs, trajid, trajtimes[trajid]); // Time int cost
      // cout << "arraygradT_container 333: " << arraygradT_container[0] << endl;
      //Get gradT gradC
      ...
      // cout << "feasibility_penalty: " << feasibility_penalty << endl;
    ...
    for(int trajid = 0; trajid < opt->trajnum; trajid++){
      ...
      // cout << "arraygradT_container 444: " << arraygradT_container[0] << endl;
      // cout << "gradP_container: " << gradP_container[0] << endl;
  ...
  void PolyTrajOptimizer::addPVAGradCost2CT(std::vector<Eigen::VectorXd>  &gdTs, Eigen::VectorXd &costs,  const int trajid, const double trajtime)
  {
    ...
    {
      double constrain_pt_t = (j == K + 1) ? T : j * delta_t_;// 0, 0.2, 0.4, ..., 6.0, 6.2, 6.3825
      ...
      if (violaVel > 0.0)
      {
        ...
        // cout << "第 " << i << " 段piece的时间gdTs为: " << gdTs[trajid](i) << endl;
        costs(2) += delta_t_ * omg_j * wei_feas_ * violaVelPena;
        // cout << "Vel cost为: " << costs(2) << endl;
      }
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        vel2_reci = 1.0 / (z_h0 * z_h0);
        vel2_reci_e = 1.0 / (z_h0 * z_h0+epis);// epis = 0
        vel3_2_reci_e = vel2_reci_e * sqrt(vel2_reci_e);
        z_h0 = 1.0 / z_h0;
        ...
        ego_R << dsigma(0), -dsigma(1),
                 dsigma(1), dsigma(0);
        ego_R = singul_ * ego_R * z_h0;// singul_ = 1
        ...
        if(violaCurR > 0.0){
          ...
          // cout << "CurR cost为: " << costs(2) << endl;
        }
      }
      t += jerkOpt_container[trajid].get_T1()(i);
      // cout << "cost 2: " << costs(2) << endl;
    }

    /*-------------------------------------------------------------------------------------------*/
    /*----------------------------fixed number sampling strategy---------------------------------*/
    // cout << "total cost: " << costs(2) << endl;
  }


  ///viola*, viola*Pena, viola*PenaD
  //对应论文中公式(63),为了确保惩罚项的连续可微性和非负性,目的就是将靠近x = 0处的函数smooth，方便梯度快速收敛(如果函数是尖的(non-smooth)，不便于梯度收敛)
  ...
  // dynamicObsGradCostP(omg_j, time_int_pena, constrain_pt_t, beta0, beta1, i, K, sigma, dsigma, ddsigma, ego_R, help_R, gdTs, trajid, trajtime);
  ...
  // 对应论文中的公式(54)，将all_dists代入log-sum-exp函数来近似max(min)函数
