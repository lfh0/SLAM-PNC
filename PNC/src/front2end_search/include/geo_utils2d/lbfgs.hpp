...
namespace lbfgs
{
     ...
     */
    // 1.lbfgs_evaluate_t->将指向回调函数的指针传递给lbfgs_optimize()
    // 2.lbfgs_stepbound_t->提供步长的外部上限(可选)
    // 3.lbfgs_progress_t->获取当前进度(变量、函数、梯度) and 根据需要停止迭代进程(可选)
    // 4.x->决策变量的向量
    // 5.f->接受变量的代价函数的最终值
    // 6.proc_evaluate->给定变量x的当前值，提供函数f（x）和梯度g（x）求值的回调函数。客户端程序必须实现与lbfgs_evaluate_t兼容的回调函数，并将指针传递给回调函数。 
    // 7.proc_stepbound->回调函数提供要搜索的步长的上限值，提供行搜索前变量的起始值，以及当前步长矢量（可以是负梯度）。客户端程序可以实现此功能，以实现更高效的行搜索。如果不使用它，只需将其设置为nullptr即可。
    // 8.proc_progress->用于接收最小化过程的进度（迭代次数、成本函数的当前值）的回调函数。如果不需要进度报告，则可以将此参数设置为nullptr。 
    // 9.instance->客户端程序的用户数据指针。回调函数将接收此参数的值。
    // 10.param->L-BFGS优化的参数。
